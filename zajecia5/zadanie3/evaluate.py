...
def create_baseline():
    ...
    model.add(Dense(10, input_dim=X_train.shape[1], activation='relu', kernel_initializer='normal'))
...
predictions_prod = estimator.predict(X_prod)

with open(os.path.join("test-A", "out.tsv"), 'w') as file:
    for prediction in predictions_prod:
        file.write(str(prediction[0]) + '\n')
